...
| `niri-minimap` | startup | For about a second after you change workspace, scroll the view, or open/close/resize a column, it shows the whole row of columns scaled down in the middle of the screen: windows at their true widths, a frame for the part on screen, `◀2 · 1▶` for what is off screen. Clicks go through it. `niri-minimap --show` shows it now; `--png FILE [WORKSPACE]` draws it to a file. | niri has no minimap. The IPC gives no view position, so the script replays niri's `center-focused-column "never"` rule to place the frame. After a touchpad swipe or a mouse drag of the view, the frame can be off until niri next scrolls to a column. |
